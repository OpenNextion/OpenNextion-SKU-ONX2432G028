...
// Verified ONX2432G028 display direction. These macros describe the intended
// panel transform; displayRotationFromConfig() maps them to Arduino_GFX IDs.
...
// Build the same simple LVGL page as the ESP-IDF touch test: white background
// and centered gray "Touch Test" text. Touch dots are drawn separately.
...
void setup() {
  ...
  delay(500);
  Serial.println();
  Serial.println("ONX2432G028 Arduino 01_touch_test");

  Wire.begin(NEXTION_IIC_SDA_PIN, NEXTION_IIC_SCL_PIN, 400000);
  lcdResetByExpander();

  // Initialize the ST7789 LCD first, then hand its flush callback to LVGL.
  ...
  // Backlight is a direct GPIO on this board.
  pinMode(LCD_BL_PIN, OUTPUT);
  digitalWrite(LCD_BL_PIN, HIGH);

  // Keep running even if the Adafruit ID check is stricter than the raw chip ID.
