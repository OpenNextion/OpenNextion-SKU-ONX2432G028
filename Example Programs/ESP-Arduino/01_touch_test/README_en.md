...
This is the ESP-Arduino version of `01_touch_test` for the ONX2432G028 2.8-inch display board. It uses standard Arduino libraries for the hardware drivers and LVGL for UI rendering:
...
3. Open `ONX2432G028/ESP-Arduino/01_touch_test/01_touch_test.ino`.
...
The default values are the direction settings verified on ONX2432G028 hardware. Arduino_GFX rotation IDs do not map one-to-one to ESP-IDF `swap_xy` / `mirror_x` / `mirror_y`, so the sketch translates the macros through `displayRotationFromConfig()`.
...
- This example does not add a custom Arduino driver library. LCD, touch, IO expander, and UI are handled by standard Arduino libraries or third-party libraries.
