...
static const lv_coord_t TEXT_WIDTH = 216;
...
static void boardBegin() {
  Serial.begin(115200);
  delay(500);
  Serial.println("\nONX2432G028 Arduino 05_wifi_test");
...
static void scanWifi() {
  ...
  for (int i = 0; i < shown; i++) {
    // On the 240 px panel, keep each row left-aligned and truncate from the
    // right. Centering long SSIDs makes the left side disappear first.
...
static void buildUi() {
  ...
  // Keep the title and Scan button on separate horizontal zones. Centering the
  // title overlaps the button on the 2.8-inch screen.
  ...
  lv_obj_set_width(statusLabel, TEXT_WIDTH);
  ...
  lv_obj_set_size(wifiList, TEXT_WIDTH, 180);
  lv_obj_align(wifiList, LV_ALIGN_TOP_MID, 0, 88);
  lv_obj_set_style_pad_all(wifiList, 4, 0);
  ...
  // Password panel is intentionally narrower than the screen and sits above the
  // keyboard so both remain usable without clipping.
  lv_obj_set_size(connectPanel, 220, 210);
  ...
  lv_obj_set_width(ssidLabel, 192);
  ...
  lv_obj_set_width(passwordTa, 192);
  ...
  lv_obj_set_size(connectBtn, 92, 42);
  lv_obj_align(connectBtn, LV_ALIGN_BOTTOM_LEFT, 10, -12);
  ...
  lv_obj_set_size(closeBtn, 78, 42);
  lv_obj_align(closeBtn, LV_ALIGN_BOTTOM_RIGHT, -10, -12);
  ...
  lv_obj_set_size(keyboard, 232, 150);
