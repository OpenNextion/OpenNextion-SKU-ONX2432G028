...
static void boardBegin() {
  Serial.begin(115200);
  delay(500);
  Serial.println("\nONX2432G028 Arduino 03_sd_card_image_test");
...
static bool isImageFile(const String &name) {
  // Keep the scan rule close to the ESP-IDF demo. Some extensions are listed
  // for directory compatibility, although this Arduino port renders JPG/PNG.
...
static void sortImages() {
  // Stable visual order makes swipe testing repeatable regardless of FAT entry
  // order on the SD card.
...
static void showCenteredMessage(const char *message, uint32_t color = RGB565_RED) {
  // Direct-draw error pages are used after LVGL is suspended for image browsing.
  // Wrap text manually so long messages still fit on the 240 px wide panel.
  static const uint8_t TEXT_SIZE = 2;
  static const uint8_t CHAR_W = 6 * TEXT_SIZE;
  static const uint8_t LINE_H = 8 * TEXT_SIZE + 4;
  int16_t maxChars = max<int16_t>(1, (gfx->width() - 24) / CHAR_W);
  String lines[4];
  uint8_t lineCount = 0;
  String word;
  ...
  while (*p && lineCount < 4) {
    if (*p == ' ' || *(p + 1) == '\0') {
      ...
      } else if ((int)(lines[lineCount].length() + 1 + word.length()) <= maxChars) {
        lines[lineCount] += " ";
        lines[lineCount] += word;
      } else if (lineCount < 3) {
        lineCount++;
  ...
  if (lineCount == 0 && lines[0].length() == 0) lines[0] = message;
  lineCount++;

  gfx->fillScreen(RGB565_WHITE);
  gfx->setTextColor(color);
  gfx->setTextSize(TEXT_SIZE);
  int16_t y = (gfx->height() - lineCount * LINE_H) / 2;
  for (uint8_t i = 0; i < lineCount; ++i) {
    int16_t x = (gfx->width() - lines[i].length() * CHAR_W) / 2;
    gfx->setCursor(max<int16_t>(0, x), y + i * LINE_H);
    gfx->print(lines[i]);
  }
}

static void showLvglPrompt(lv_obj_t *scr, const char *message) {
  // All initial abnormal pages share the same geometry. Keep the label narrower
  // than the 240 px panel so LVGL can wrap English text without clipping.
  lv_obj_set_style_bg_color(scr, lv_color_white(), 0);
  lv_obj_t *label = lv_label_create(scr);
  lv_obj_set_width(label, gfx->width() - 24);
...
static bool readWholeFile(const String &path, uint8_t **data, size_t *size) {
  ...
  *size = 0;
  ...
  // Decode libraries expect contiguous input. Use PSRAM so larger images do not
  // consume the ESP32-S3 internal SRAM needed by LVGL and the SD stack.
...
static bool drawJpegImage(const String &path) {
  ...
  // Query the original dimensions first for logging and to size the decoder
  // output buffer correctly before the actual JPEG decode.
  ...
  // This keeps same-size images visually consistent on the display width.
...
static void pollImageTouch() {
  ...
  // Horizontal swipes change image. The threshold avoids accidental changes
  // from taps or mostly vertical drags.
...
static void scanImages() {
  ...
  // Store only file names. Full paths are rebuilt later so the array stays
  // compact and path handling matches the ESP-IDF example's /images folder.
...
static void buildUi() {
  ...
  if (!sdReady) {
    showLvglPrompt(scr, "No SD card detected. Please insert it and then power on again.");
  } else if (imageCount == 0) {
    showLvglPrompt(scr, "Image not found.");
  } else {
...
void setup() {
  ...
  scanImages();
  buildUi();
  // Render the initial LVGL prompt immediately when there is no image browser.
...
void loop() {
  ...
  lastMs = now;
  if (imageBrowserMode) {
    // In image mode the LCD is owned by the direct renderer, not LVGL.
    pollImageTouch();
  } else {
    lv_timer_handler();
  }
