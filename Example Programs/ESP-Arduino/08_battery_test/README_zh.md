...
本示例是 `08_battery_test` 的 ESP-Arduino 版本，用于测试 ONX2432G028 的电池电压采样、电量估算和充电状态显示。
...
08 例程在 ONX2432G028 实机上已验证最终显示方向。代码保留和其它例程一致的方向宏，同时使用 Arduino_GFX 的实机确认 rotation：
...
`DISPLAY_ROTATION_OVERRIDE` 用于固定 08 电池页面最终确认的物理方向，避免不同 Arduino_GFX rotation / mirror 组合在该页面上表现不一致。
