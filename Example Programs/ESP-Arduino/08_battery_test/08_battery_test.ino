...
// Verified ONX2432G028 display direction. Keep these values aligned with
// 01_touch_test so all examples share the same panel transform.
#define DISPLAY_SWAP_XY false
#define DISPLAY_MIRROR_X false
#define DISPLAY_MIRROR_Y true
// Final ONX2432G028 battery-page orientation verified on hardware. Arduino_GFX
// rotation 0 matches the physical direction after the 08 page-specific checks.
...
#ifdef DISPLAY_ROTATION_OVERRIDE
  // Keep the verified 08 setting explicit; the macro mapping below remains as
  // documentation for the same switch style used by the other examples.
  return DISPLAY_ROTATION_OVERRIDE;
#endif
  // Arduino_GFX and ESP-IDF encode ST7789 mirror/rotation differently, so keep
  // the user-facing macros above and translate them in one place.
...
static void boardBegin() {
  Serial.begin(115200);
  delay(500);
  Serial.println("\nONX2432G028 Arduino 08_battery_test");
  ...
  uint8_t rotation = displayRotationFromConfig();
  gfx->setRotation(rotation);
  Serial.printf("Display rotation=%u size=%dx%d\r\n", rotation, gfx->width(), gfx->height());
...
static void buildUi() {
  ...
  lv_obj_set_size(levelBar, 216, 20);
