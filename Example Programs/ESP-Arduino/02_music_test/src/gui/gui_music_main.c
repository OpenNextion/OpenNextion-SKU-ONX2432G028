...
/* The original ESP-IDF resources are kept, but the cover image is zoomed down
 * for the ONX2432G028 240 x 320 panel so the Music Demo layout fits cleanly. */
#define LV_IMG_ZOOM_VALUE   (192)
...
/* New helper: UI-only resume logic, without controlling the audio HAL. */
static void music_ui_resume_only(void);
static void make_imgbtn_flat(lv_obj_t * btn);
...
static void make_imgbtn_flat(lv_obj_t * btn)
{
    ...
    for(size_t i = 0; i < sizeof(sel_list) / sizeof(sel_list[0]); i++) {
        uint32_t sel = sel_list[i];
        ...
        lv_obj_set_style_pad_all(btn, 0, sel);
    }

    lv_obj_set_style_transform_zoom(btn, LV_IMG_ZOOM_NONE, LV_PART_MAIN | LV_STATE_PRESSED);
    lv_obj_set_style_transform_angle(btn, 0, LV_PART_MAIN | LV_STATE_PRESSED);
}
...
lv_obj_t * _lv_demo_music_main_create(lv_obj_t * parent)
{
    ...
    /* Portrait layout for the 2.8-inch board. The row proportions are tightened
     * compared with the 3.5-inch example to keep the player page in bounds. */
    static const lv_coord_t grid_cols[] = {LV_GRID_FR(1), LV_GRID_TEMPLATE_LAST};
...
# endif
    ...
    lv_anim_set_values(&a, 1, LV_IMG_ZOOM_VALUE);
    ...
    lv_obj_align_to(title, logo, LV_ALIGN_CENTER, 0, 100);

    lv_anim_set_path_cb(&a, lv_anim_path_ease_in);
    lv_anim_set_var(&a, logo);
    lv_anim_set_time(&a, 400);
    lv_anim_set_delay(&a, INTRO_TIME + 800);
    lv_anim_set_values(&a, LV_IMG_ZOOM_VALUE, 10);
...
/* Previous/next track: use gui_hal_get_track_count() to wrap around. */
void _lv_demo_music_album_next(bool next)
{
    uint32_t track_cnt = gui_hal_get_track_count();
    if(track_cnt == 0) return;
    ...
    if(next) {
        /* Next track */
        id = (id + 1) % track_cnt;
        gui_hal_prev_next(1);     /* Notify HAL to switch to next track */
    }
    else {
        /* Previous track */
        if(id == 0) {
        ...
        gui_hal_prev_next(0);     /* Notify HAL to switch to previous track */
    }

    track_load(id);              /* Update LVGL UI */

    if (playing) {
        music_ui_resume_only();
    }
}

/* Select a track from list/cover: update UI only, HAL is controlled elsewhere. */
void _lv_demo_music_play(uint32_t id)
{
    track_load(id);
    music_ui_resume_only();      /* Switch UI to "playing" state (spectrum/slider) */
}

/* Play button click: control both UI and HAL player. */
void _lv_demo_music_resume(void)
{
    music_ui_resume_only();      /* 1. First switch UI to "playing" state. */
    gui_hal_toggle_play_pause(); /* 2. Then toggle HAL to play/resume. */
}
...
void _lv_demo_music_pause(void)
{
    ...
    lv_img_set_zoom(album_img_obj, LV_IMG_ZOOM_VALUE);
    lv_timer_pause(sec_counter_timer);
    lv_obj_clear_state(play_obj, LV_STATE_CHECKED);

    gui_hal_toggle_play_pause();         /* Notify HAL to pause playback. */
}
...
/* UI-only resume helper: restart animations, slider and button state. */
static void music_ui_resume_only(void)
{
    lv_anim_del(spectrum_obj, spectrum_anim_cb);
...
static lv_obj_t * create_ctrl_box(lv_obj_t * parent)
{
...
#else
    lv_obj_set_style_pad_bottom(cont, 3, 0);
#endif
    /* Keep the transport controls on separate grid islands. 240 px is narrow,
     * so using spacer columns between prev/play/next makes touch targets less
     * likely to be confused without changing the ESP-IDF visual assets. */
    static const lv_coord_t grid_col[] = {LV_GRID_FR(1), LV_GRID_FR(5), LV_GRID_FR(2), LV_GRID_FR(5), LV_GRID_FR(2), LV_GRID_FR(5), LV_GRID_FR(1), LV_GRID_TEMPLATE_LAST};
    ...
    lv_obj_set_grid_cell(icon, LV_GRID_ALIGN_END, 6, 1, LV_GRID_ALIGN_CENTER, 0, 1);
    lv_obj_add_flag(icon,LV_OBJ_FLAG_HIDDEN);
    ...
    lv_obj_clear_flag(slider_obj, LV_OBJ_FLAG_CLICKABLE);
    


#if LV_DEMO_MUSIC_LARGE == 0
    lv_obj_set_height(slider_obj, 2);
#else
    ...
    lv_obj_set_grid_cell(slider_obj, LV_GRID_ALIGN_STRETCH, 1, 4, LV_GRID_ALIGN_START, 1, 1);
    lv_obj_set_grid_cell(time_obj, LV_GRID_ALIGN_END, 5, 1, LV_GRID_ALIGN_START, 1, 1);
    lv_obj_set_style_translate_y(time_obj, -4, 0);
...
static void track_load(uint32_t id)
{
    ...
    lv_anim_set_var(&a, album_img_obj);
    lv_anim_set_time(&a, 500);
    lv_anim_set_values(&a, LV_IMG_ZOOM_VALUE, LV_IMG_ZOOM_VALUE / 2);
    lv_anim_set_exec_cb(&a, _img_set_zoom_anim_cb);
    lv_anim_set_ready_cb(&a, NULL);
    lv_anim_start(&a);
    ...
    lv_anim_set_values(&a, LV_IMG_ZOOM_VALUE / 4, LV_IMG_ZOOM_VALUE);
    lv_anim_set_exec_cb(&a, _img_set_zoom_anim_cb);
    lv_anim_set_ready_cb(&a, NULL);
    lv_anim_start(&a);
...
static void spectrum_anim_cb(void * a, int32_t v)
{
    ...
    static int32_t zoom_ofs = 0;
    const int32_t ZOOM_MAX_OFS = 3;
    const int32_t BASS_TH = 16;
    const int32_t DECAY = 6;

    if(spectrum[spectrum_i][0] >= BASS_TH) {
        zoom_ofs = ZOOM_MAX_OFS;
    }
    else {
        zoom_ofs -= DECAY;
        if(zoom_ofs < 0) zoom_ofs = 0;
    }

    lv_img_set_zoom(album_img_obj, (uint16_t)(LV_IMG_ZOOM_VALUE + zoom_ofs));
}
...
static lv_obj_t * album_img_create(lv_obj_t * parent)
{
    ...
    /* Support track_id >= 3 by using track_id % 3 to select cover and spectrum data. */
    uint32_t cover_index = track_id % 3;
...
static void timer_cb(lv_timer_t * t)
{
    ...
    /* 1. Handle "auto-next" events coming from the audio HAL. */
    uint32_t new_id;
    while (gui_hal_fetch_track_end(&new_id)) {
        /* Synchronize the UI to the new track:
         * - Update title / artist / cover / list selection
         * - Reset time_act and slider
         */
        track_load(new_id);

        /* If the UI is in "playing" state, restart animations and progress. */
        if (playing) {
            music_ui_resume_only();
        }
    }

    /* 2. Update time label and slider while playing. */
    if (playing) {
        time_act++;
        lv_label_set_text_fmt(time_obj,
                              "%"LV_PRIu32":%02"LV_PRIu32,
...
static void spectrum_end_cb(lv_anim_t * a)
{
    ...
    /* If playback is paused, do not restart the animation. */
    if (!playing) {
        return;
    }

    if(!album_img_obj || spectrum_len == 0) 
    {
        return;
    }

    /* Restart spectrum animation from the beginning. */
    spectrum_i       = 0;
    spectrum_i_pause = 0;
    ...
    /* Keep the same frame rate as in _lv_demo_music_resume (30 FPS). */
    lv_anim_set_time(&anim, (spectrum_len * 1000) / 30);
    lv_anim_set_playback_time(&anim, 0);

    /* Loop the spectrum animation by reusing this callback. */
