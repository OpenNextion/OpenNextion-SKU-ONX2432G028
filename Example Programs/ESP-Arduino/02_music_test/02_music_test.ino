...
// Display direction switches kept consistent with 01_touch_test. They are
// converted to the Arduino_GFX rotation value by displayRotationFromConfig().
...
static void setExpanderPin(uint8_t pin, bool level) {
  // All PCF8574 writes go through the shadow byte so unrelated output bits keep
  // their last state. This is especially important for I2S_CTRL pop suppression.
...
static void lcdResetByExpander() {
  ...
  ioExpander.digitalWriteByte(expanderState);
  setExpanderPin(PCF8574_PIN_I2S_CTRL, false);
  delay(20);
...
static void lvglFlush(lv_disp_drv_t *drv, const lv_area_t *area, lv_color_t *colorP) {
  // LVGL renders into a small line buffer; Arduino_GFX pushes each invalidated
  // area to the ST7789 panel using the color byte order selected in build_opt.h.
...
static void lvglTouchRead(lv_indev_drv_t *, lv_indev_data_t *data) {
  ...
  if (touch.touched()) {
    // The CST826 coordinates already match the configured panel direction, so
    // only clamp them to LVGL's active resolution here.
...
static void lvglBegin() {
  lv_init();
  // A 40-line buffer keeps SRAM usage reasonable while still avoiding excessive
  // tiny flushes on the 240 x 320 screen.
...
static void boardBegin() {
  ...
  // The expander is first accessed at 100 kHz, matching the ESP-IDF example and
  // reducing the chance of an amplifier enable glitch during power-up.
  Wire.begin(NEXTION_IIC_SDA_PIN, NEXTION_IIC_SCL_PIN, PCF8574_I2C_FREQ);
  lcdResetByExpander();
  Wire.setClock(TOUCH_I2C_FREQ);
  Serial.println("\nONX2432G028 Arduino 02_music_test");
...
void loop() {
  ...
  uint32_t now = millis();
  // LVGL does not own the Arduino main loop, so feed its tick and timers here.
