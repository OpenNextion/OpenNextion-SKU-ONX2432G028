...
static const uint32_t PCF8574_I2C_FREQ = 100000;
static const uint32_t TOUCH_I2C_FREQ = 400000;
...
// PCF8574 ports default high after power-up. Keep I2S_CTRL low in the shadow
// byte from the first expander write so the NS4168 amplifier never glitches on
// during LCD reset or other PCF8574 pin changes.
...
static void lcdResetByExpander() {
  ...
  // Apply the safe shadow state first; unlike 0xFF, this keeps I2S_CTRL LOW.
  ioExpander.digitalWriteByte(expanderState);
  delay(20);
...
static void boardBegin() {
  // Match 02_music_test: touch PCF8574 first at 100 kHz and apply the safe
  // shadow byte immediately, then speed the shared I2C bus back up for touch.
  Wire.begin(NEXTION_IIC_SDA_PIN, NEXTION_IIC_SCL_PIN, PCF8574_I2C_FREQ);
  lcdResetByExpander();
  speakerAmp(false);
  Wire.setClock(TOUCH_I2C_FREQ);

  Serial.begin(115200);
  Serial.println("\nONX2432G028 Arduino 07_microphone_test");
...
static bool speakerBegin() {
  // Speaker TX is deliberately created lazily. Initializing I2S STD at boot
  // switches BCLK/WS/DOUT into peripheral mode and was audible on this board.
  if (speakerTx) return true;
...
static void enableSpeakerTx() {
  // First playback opens the speaker path; idle/recording states keep it closed.
  if (!speakerReady) {
    speakerReady = speakerBegin();
    if (!speakerReady) return;
  }
  if (!speakerReady || speakerEnabled) return;
  esp_err_t err = i2s_channel_enable(speakerTx);
  if (err == ESP_OK) {
    speakerEnabled = true;
  } else {
...
static void disableSpeakerTx() {
  if (!speakerReady || !speakerEnabled) return;
  i2s_channel_disable(speakerTx);
  speakerEnabled = false;
}

static void writeSpeakerSilence(size_t samples = AUDIO_CHUNK_SAMPLES) {
  if (!speakerReady || !speakerEnabled) return;
  static int16_t silence[AUDIO_CHUNK_SAMPLES] = {};
  size_t bytesWritten = 0;
  i2s_channel_write(speakerTx, silence, min(samples, AUDIO_CHUNK_SAMPLES) * sizeof(int16_t), &bytesWritten, 20);
}
...
static void stopAudio() {
  ...
  // Mute first, then push/stop the digital output so STOP does not leave hiss
  // on the speaker. This mirrors the pop-suppression flow used by 02_music_test.
  speakerAmp(false);
  writeSpeakerSilence();
  disableSpeakerTx();
...
static void startPlayback() {
  if (!recordReady || recordSamples == 0) {
    stopAudio();
    return;
  }
  playIndex = 0;
  ...
  // Start I2S and feed a short silent lead-in before enabling the amplifier.
  enableSpeakerTx();
  if (!speakerEnabled) {
    stopAudio();
    lv_label_set_text(statusLabel, "Speaker init failed");
    return;
  }
  writeSpeakerSilence();
  delay(20);
...
static void buildUi() {
  ...
  lv_label_set_text(statusLabel, (micReady && recordBuffer) ? "Press and hold MIC to record." : "Audio init failed");
...
static void processPlayback() {
  if (audioState != AudioState::Playing || !recordReady || !recordBuffer || !speakerEnabled) return;
...
void setup() {
  ...
  micReady = micBegin();
  // Do not initialize speaker I2S at boot. The BCLK/WS/DOUT mux transition can
  // create an audible transient, so speaker output is opened lazily on playback.
  speakerReady = false;
  speakerAmp(false);
  buildUi();
  if (!bufferReady || !micReady) {
