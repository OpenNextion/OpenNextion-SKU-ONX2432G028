 ...
 * @brief Customized LVGL widgets demo ported from the ONX2432G028 ESP-IDF
 *        out-of-box example. Keep UI/layout changes here aligned with the
 *        ESP-IDF source so the Arduino example preserves the same effect.
 *
 */
...
static void profile_create(lv_obj_t * parent)
{
    ...
    LV_IMG_DECLARE(Nextionblue);

    lv_obj_t * avatar = lv_img_create(panel1);
    lv_img_set_src(avatar, &Nextionblue);

    lv_obj_t * name = lv_label_create(panel1);
    lv_label_set_text(name, "Open Nextion");
    lv_obj_add_style(name, &style_title, 0);

    lv_obj_t * dsc = lv_label_create(panel1);
    lv_obj_add_style(dsc, &style_text_muted, 0);
    lv_label_set_text(dsc, "Open Nextion Offers an All-in-One Solution for Interactive Smart Device");
    ...
    lv_label_set_text(email_label, "support@itead.cc");
    ...
    lv_label_set_text(call_label, "+86-755-27955416");
    ...
    if(code == LV_EVENT_FOCUSED) {
        if(lv_indev_get_type(lv_indev_get_act()) == LV_INDEV_TYPE_POINTER) {
            if(calendar == NULL) {
                ...
                #if NEXTION_3_5
                if(disp_size == DISP_SMALL) lv_obj_set_size(calendar, 220, 300);
                #endif
                #if NEXTION_2_8
                if(disp_size == DISP_SMALL) lv_obj_set_size(calendar, 200, 250);
                #endif
                else if(disp_size == DISP_MEDIUM) lv_obj_set_size(calendar, 200, 220);
                else  lv_obj_set_size(calendar, 220, 280);
