...
// Final ONX2432G028 out-of-box demo orientation verified on hardware.
// Keep the switch style consistent with the other examples so direction changes
// can still be made through the familiar swap/mirror macros.
...
// Arduino_GFX drives the ST7789 panel; LVGL renders into the line buffer below.
...
static uint8_t displayRotationFromConfig() {
  // Arduino_GFX rotation IDs combine rotation and mirror bits. The ESP-IDF
  // example exposes swap_xy/mirror_x/mirror_y, so translate that model here.
...
static void lcdResetByExpander() {
  ...
  // Match the ESP-IDF board bring-up: release all PCF8574 pins, pulse LCD_RST,
  // then leave the panel out of reset before Arduino_GFX initialization.
...
static void boardBegin() {
  Serial.begin(115200);
  delay(500);
  Serial.println("\nONX2432G028 Arduino 09_outofbox_demo");
...
void setup() {
  boardBegin();
  // This is the ESP-IDF example's customized LVGL widgets demo. Keeping it as
  // a separate module avoids recreating the UI by hand in the Arduino sketch.
  lv_demo_widgets();
}
