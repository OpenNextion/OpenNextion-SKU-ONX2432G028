...
// ONX2432G028 is 240 px wide. Keep status text below the full width so long
// English strings can wrap with left/right margin instead of clipping.
...
static uint8_t displayRotationFromConfig() {
  // Arduino_GFX encodes mirror/rotation differently from the ST7789 panel command
...
static void boardBegin() {
  Serial.begin(115200);
  delay(500);
  Serial.println("\nONX2432G028 Arduino 04_sd_card_test");
...
static void setResult(const char *result, uint32_t color, const char *detail) {
  // Both labels are width-limited and wrapped in buildUi(), so every test result
  // path, including "NO SD CARD DETECTED", stays inside the 2.8-inch screen.
...
static void buildUi() {
  ...
  lv_obj_set_width(resultLabel, TEXT_WIDTH);
  lv_label_set_long_mode(resultLabel, LV_LABEL_LONG_WRAP);
  ...
  lv_obj_align(resultLabel, LV_ALIGN_CENTER, 0, -86);
  ...
  lv_obj_set_width(detailLabel, TEXT_WIDTH);
  ...
  lv_obj_align(detailLabel, LV_ALIGN_CENTER, 0, 82);
}
