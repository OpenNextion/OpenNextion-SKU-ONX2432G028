...
The ONX2432G028 display is 240 pixels wide. Result and detail labels are limited to 216 pixels and use LVGL word wrapping so long strings such as `NO SD CARD DETECTED` and file paths do not overflow the screen.
...
- The SD card is remounted on every button tap, so removing the card after one successful test shows `NO SD CARD DETECTED` on the next tap.

## Display And Touch

- Display direction matches `01_touch_test`: `DISPLAY_SWAP_XY=false`, `DISPLAY_MIRROR_X=true`, `DISPLAY_MIRROR_Y=false`.
- The result page uses a white background and centered text.
- Keep all user-visible text narrower than the screen and enable wrapping; avoid fixed text widths of 240 px or more on this 2.8-inch board.
