...
#define TEST_ADC_CHANNEL        (ADC_CHANNEL_3)  // GPIO4
// NOTE: Keep these values consistent with your schematic.
// R31 = 1.0M (upper), R32 = 2.2M (lower)

#define TEST_RESISTOR_UPPER     (1050000)        // R31 1M   
#define TEST_RESISTOR_LOWER     (2150000)        // R32 2.2M

const char *TAG = "BATTERY_TEST";
...
// Forward declaration (used by battery_estimation_reset)
static void battery_init(void);

// Cache charging level so the battery estimation module can query charging state
// without doing an extra I2C read (we update this in the battery task loop).
static volatile exio_pcf8574_level s_charging_level = EXIO_PCF8574_LEVEL_L; // H=charging

static bool battery_is_charging(void *user_data)
{
    const exio_pcf8574_level *lvl = (const exio_pcf8574_level *)user_data;
    return (lvl != NULL) && (*lvl == EXIO_PCF8574_LEVEL_H);
}

// Reset the estimation filter/state when charging state changes (USB plug/unplug)
static void battery_estimation_reset(void)
{
    if (adc_battery_estimation_handle) {
        adc_battery_estimation_destroy(adc_battery_estimation_handle);
        adc_battery_estimation_handle = NULL;
    }
    battery_init();
}

static lv_obj_t *battery_test_label;   
 ...
 * 
 * This handle manages the LCD panel (e.g., ST7789) hardware operations, including 
...
/**
 * @brief Initialize ST7789 LCD panel via SPI interface
 * 
 * This function performs full initialization of the ST7789 LCD panel, including hardware reset via PCF8574 expander,
...
esp_err_t Lcd_Init(void)
{
    ...
    esp_lcd_panel_dev_config_t panel_config = {
        ...
        .rgb_ele_order  = LCD_RGB_ELEMENT_ORDER_RGB,    /* RGB color element order */
        .bits_per_pixel = 16,                           /* Color depth (16 bits per pixel, RGB565) */
    };


    /* Create ST7789 LCD panel handle and bind to the SPI IO handle */
...
void lv_port_init(void)
{
    ...
    lvgl_port_display_cfg_t display_cfg = {
        ...
        .rotation = {                        // Display rotation/mirror configuration
            .swap_xy = 0,                    // Do not swap X nad Y axes (no 90/270° rotation)
            .mirror_x = 0,                   // Do not mirror display horizontally (no flip left-right)
...
/**
 * @brief Initialize ADC-based battery capacity estimation module
 * 
 * This function configures the necessary parameters for battery voltage sampling and capacity calculation,
 * then creates a battery estimation handle to manage the sampling process.
 * The module uses an ADC to sample the battery voltage (via a voltage divider circuit)
 * and estimates the remaining battery capacity based on the sampled voltage.
 */
static void battery_init(void)
{
    // Configuration structure for ADC battery estimation (contains ADC settings and voltage divider parameters)
    adc_battery_estimation_t config = {
        .internal = {
            .adc_unit     = TEST_ADC_UNIT,    // ADC unit to use (e.g., ADC1/ADC2, defined by TEST_ADC_UNIT macro)
            .adc_bitwidth = TEST_ADC_BITWIDTH,// ADC sampling bit width (e.g., 12-bit, defined by TEST_ADC_BITWIDTH macro)
            .adc_atten    = TEST_ADC_ATTEN,   // ADC input attenuation (adjusts measurable voltage range, defined by TEST_ADC_ATTEN macro)
        },
        .adc_channel      = TEST_ADC_CHANNEL,  // Specific ADC channel used for battery voltage sampling (defined by TEST_ADC_CHANNEL macro)
        .lower_resistor   = TEST_RESISTOR_LOWER,// Resistance value of the lower resistor in the voltage divider (defined by TEST_RESISTOR_LOWER macro)
        .upper_resistor   = TEST_RESISTOR_UPPER,// Resistance value of the upper resistor in the voltage divider (defined by TEST_RESISTOR_UPPER macro)

        // IMPORTANT: Provide charging state to avoid "discharging monotonic clamp" locking SOC.
        // CHG_N is HIGH when charging (as you confirmed).
        .charging_detect_cb = battery_is_charging,
        .charging_detect_user_data = (void *)&s_charging_level,
    };

    // Create battery estimation handle with the above configuration
    // The handle is stored in a global variable (adc_battery_estimation_handle) for later use in sampling/capacity calculation
...
/**
 * @brief Select corresponding LVGL battery symbol based on battery level
 * @param battery_level Current battery capacity percentage (0.0f ~ 100.0f)
 * @return LVGL battery symbol string (e.g., LV_SYMBOL_BATTERY_EMPTY)
 */
static const char* select_battery_symbol(float battery_level) {
    if (battery_level < 25.0f) {
        return LV_SYMBOL_BATTERY_EMPTY;
    } else if (battery_level < 50.0f) {
        return LV_SYMBOL_BATTERY_1;
    } else if (battery_level < 75.0f) {
        return LV_SYMBOL_BATTERY_2;
    } else if (battery_level < 99.5f) {
...
/**
 * @brief FreeRTOS task for battery status monitoring and LVGL display
 * @param input_data Unused input parameter (can pass configuration data if needed)
 * 
 * Function workflow:
 * 1. Create LVGL labels for test status and battery information display
 * 2. Continuously read battery level via ADC and charging status via PCF8574
 * 3. Update LVGL UI with corresponding battery icon, percentage and color
 * 4. Handle errors (e.g., ADC read failure, PCF8574 communication error)
 */
void battery_test(void *input_data)
{
    float battery_level = 0.0f;                          // Current battery capacity percentage (0.0f ~ 100.0f)
    esp_err_t adc_err = ESP_FAIL;                        // ADC read error code
    esp_err_t pcf_err = ESP_FAIL;                        // PCF8574 read error code
    exio_pcf8574_level charging_status = EXIO_PCF8574_LEVEL_H;  // Charging status (H=charging, L=discharging per hardware logic)
    char battery_percent_buf[10] = {0};                  // Buffer for battery percentage string (e.g., "75%")
    const char* battery_symbol = LV_SYMBOL_BATTERY_EMPTY;// Selected battery icon symbol

    // Create test status label (center of screen)
    if (lvgl_port_lock(0)) 
    {  // Acquire LVGL lock for thread-safe UI operation
        battery_test_label = lv_label_create(lv_scr_act());
        lv_label_set_recolor(battery_test_label, true);    // Enable text color recoding via #color syntax
        lv_label_set_text(battery_test_label, "#808080 Battery Test#");
        lv_obj_center(battery_test_label);                 // Align label to screen center

        // Create battery status label (top-right corner)
        battery_status_label = lv_label_create(lv_scr_act());
        lv_obj_set_size(battery_status_label, 80, 30);
        lv_obj_align(battery_status_label, LV_ALIGN_TOP_RIGHT, 0, 0);  // Align to top-right
        lv_obj_set_style_text_align(battery_status_label, LV_TEXT_ALIGN_LEFT, LV_PART_MAIN|LV_STATE_DEFAULT);
        lvgl_port_unlock();  // Release LVGL lock after UI creation
    }

    // Track charging state changes to reset estimation filter (USB plug/unplug)
    bool has_last_chg = false;
    exio_pcf8574_level last_charging_status = EXIO_PCF8574_LEVEL_L;
    int settle_cycles = 0; // Skip UI update for a couple cycles after state change

    // Infinite loop for continuous battery monitoring
    while (1) {
        // 1) Read charging status first, update cached level for the estimation callback
        pcf_err = exio_pcf8574_pin_read(EXIO_PCF8574_PIN_CHG_N, &charging_status);
        if (pcf_err == ESP_OK) {
            s_charging_level = charging_status;
            if (!has_last_chg) {
                last_charging_status = charging_status;
                has_last_chg = true;
            } else if (charging_status != last_charging_status) {
                // Treat charge-state change as USB plug/unplug or charge-state transition.
                ESP_LOGI(TAG, "Charging state changed (%s -> %s), reset estimation filter", 
                         (last_charging_status == EXIO_PCF8574_LEVEL_H) ? "CHARGING" : "NOT_CHARGING",
                         (charging_status == EXIO_PCF8574_LEVEL_H) ? "CHARGING" : "NOT_CHARGING");
                last_charging_status = charging_status;
                battery_estimation_reset();
                settle_cycles = 2; // 2 * 500ms = 1s
            }
        }

        // 2) Read battery level via ADC (from battery estimation module)
        if (adc_battery_estimation_handle) {
            adc_err = adc_battery_estimation_get_capacity(adc_battery_estimation_handle, &battery_level);
        } else {
            adc_err = ESP_FAIL;
        }

        // Update UI only if both ADC and PCF8574 read successfully
        if (adc_err == ESP_OK && pcf_err == ESP_OK) 
        {
            // Format battery percentage (e.g., "75%") with safe buffer handling
            snprintf(battery_percent_buf, sizeof(battery_percent_buf)-1, "%.0f%%", battery_level);
            // Select corresponding battery icon based on capacity
            battery_symbol = select_battery_symbol(battery_level);

            // Optionally hold UI update briefly after a charge-state transition to avoid visual "jump"
            if (settle_cycles > 0) {
                settle_cycles--;
            ...
            if (lvgl_port_lock(0)) 
            {  // Acquire LVGL lock for UI update
                // Case 1: Charging (hardware logic: EXIO_PCF8574_LEVEL_H = charging)
                if (charging_status == EXIO_PCF8574_LEVEL_H) 
                {
                    lv_obj_set_style_text_color(battery_status_label, lv_palette_main(LV_PALETTE_GREEN), 0);  // Green text for charging
                    // Display: Charge symbol + Battery icon + Percentage
                    lv_label_set_text_fmt(battery_status_label, "%s %s", battery_symbol,LV_SYMBOL_CHARGE);
                }
                // Case 2: Discharging (hardware logic: EXIO_PCF8574_LEVEL_L = not charging)
                else if (charging_status == EXIO_PCF8574_LEVEL_L) 
                {
                    lv_obj_set_style_text_color(battery_status_label, lv_color_hex(0x000000), 0);  // Black text for discharging
                    // Display: Battery icon + Percentage
                    lv_label_set_text_fmt(battery_status_label, "%s %s", battery_symbol, battery_percent_buf);
                }
                lvgl_port_unlock();  // Release LVGL lock after update
            }

            ESP_LOGI(TAG, "Battery Status - Level: %s, Charging: %s", 
                     battery_percent_buf, 
                     (charging_status == EXIO_PCF8574_LEVEL_H) ? "Yes" : "No");
        }
        // Error handling: ADC or PCF8574 read failed
        else 
        {
            if (lvgl_port_lock(0)) 
            {
                lv_label_set_text_fmt(battery_status_label, "%s BATTERY ERROR", LV_SYMBOL_BATTERY_EMPTY);
                lv_obj_set_style_text_color(battery_status_label, lv_palette_main(LV_PALETTE_RED), 0);  // Red text for error
                lvgl_port_unlock();
            }
            ESP_LOGE(TAG, "Battery Read Error - ADC Err: 0x%x, PCF8574 Err: 0x%x", adc_err, pcf_err);
        }

        // Delay 500ms (refresh rate: 2Hz)
        vTaskDelay(pdMS_TO_TICKS(500));
    }
}
...
void app_main(void)
{
    ...
    ESP_ERROR_CHECK(Lcd_Init());     // Initialize ST7789 LCD panel via SPI interface


    lcd_cst826_touch_Init();             // Initialize CST826 touch controller via I2C
    // Initialize LVGL port (prepares LVGL library for use, including display and input device setup)
    // This function typically initializes the display driver, input devices (e.g., touch), and LVGL core services
    lv_port_init();
    ...
    // Set the battery charge detection pin to high level via PCF8574 IO expander
    // Purpose: Ensure the pin is configured as an input (PCF8574 pins are often set to input by pulling high internally)
    exio_pcf8574_pin_write(EXIO_PCF8574_PIN_CHG_N, EXIO_PCF8574_LEVEL_H);

    // Initialize the battery monitoring module
    // This configures ADC parameters, voltage divider resistors, and creates the battery estimation handle
    battery_init();

    // Create a FreeRTOS task for battery status monitoring and UI updates
    // - Task function: battery_test (handles reading battery level, charging status, and updating LVGL display)
    // - Task name: "battery_test" (for debugging/monitoring)
    // - Stack size: 4096 bytes (sufficient for LVGL operations and ADC/IO reads)
    // - Priority: 10 (higher than normal tasks to ensure timely status updates)
    // - No input parameters (NULL) and no task handle returned (last NULL)
    xTaskCreate(battery_test, "battery_test", 4096, NULL, 10, NULL);
