...
/**
 * @brief Initializes the ESP32 camera port with specified configurations
 * @param i2c_port I2C port number (unused in this implementation but reserved for potential I2C control)
 */
void esp_camera_port_init(i2c_port_num_t i2c_port) 
{
    camera_config_t config;  // Structure to hold camera hardware and operational configurations

    // Configure LEDC (Light Emitting Diode Controller) for camera clock control
    config.ledc_channel = CAM_LEDC_CHANNEL;  // LEDC channel used for camera clock
    config.ledc_timer = CAM_LEDC_TIMER;      // LEDC timer associated with the channel

    // Set data bus pins (D0-D7) for camera image data transmission
    ...
    // Set control pins for camera synchronization and clock
    config.pin_xclk = XCLK_GPIO_NUM;    // External clock pin (powers camera timing)
    config.pin_pclk = PCLK_GPIO_NUM;    // Pixel clock pin (synchronizes pixel data)
    config.pin_vsync = VSYNC_GPIO_NUM;  // Vertical sync pin (marks start of new frame)
    config.pin_href = HREF_GPIO_NUM;    // Horizontal reference pin (marks valid line data)

    // SCCB (Serial Camera Control Bus) configuration (I2C-like for camera control)
    config.pin_sccb_sda = -1;    // SCCB SDA pin (-1 means not used in this setup)
    config.pin_sccb_scl = -1;    // SCCB SCL pin (-1 means not used in this setup)
    config.sccb_i2c_port = 0;    // I2C port assigned to SCCB (0 as default)

    // Power management pins
    config.pin_pwdn = PWDN_GPIO_NUM;  // Power-down pin (controls camera power state)
    config.pin_reset = RESET_GPIO_NUM; // Reset pin (resets camera hardware)

    // Clock and image parameters
    config.xclk_freq_hz = 20000000;    // External clock frequency (20MHz)
    config.frame_size = FRAMESIZE_HVGA; // Capture frame size
    config.pixel_format = PIXFORMAT_RGB565; // Pixel format (RGB565 for direct display streaming)
    config.grab_mode = CAMERA_GRAB_WHEN_EMPTY; // Frame grab mode (capture when buffer is empty)
    config.fb_location = CAMERA_FB_IN_PSRAM;   // Frame buffer storage (use PSRAM to save internal RAM)
    config.jpeg_quality = 12; // JPEG compression quality (0-63; lower = better quality, unused for RGB565)
    config.fb_count = 2;      // Number of frame buffers (double buffering to reduce lag)

    exio_pcf8574_pin_write(EXIO_PCF8574_PIN_CAM_PWDN, EXIO_PCF8574_LEVEL_L);

    // Initialize the camera with the above configuration
    esp_err_t err = esp_camera_init(&config);
    
    // Check if initialization failed and print error
    if (err != ESP_OK)
    {
    ...
    // Get pointer to camera sensor for additional configuration
    sensor_t *s = esp_camera_sensor_get();
    // vertical flip (1 = flip image vertically; 0 = disable)
    s->set_vflip(s, 0);
}
...
/**
 * @brief FreeRTOS task for capturing camera frames and displaying them on LVGL UI
 * @param arg Unused task parameter (can be used to pass data to the task if needed)
 */
void camera_task(void *arg)
{
    camera_fb_t *pic;                  // Pointer to hold the camera frame buffer (stores captured image data)
    lv_img_dsc_t img_dsc;              // LVGL image descriptor structure (defines image properties for LVGL rendering)

    // Initialize LVGL image descriptor properties
    img_dsc.header.always_zero = 0;    // Reserved field, typically set to 0 in LVGL image headers

    img_dsc.header.w = 480;            // Image width in pixels (matches camera output width)
    img_dsc.header.h = 320;            // Image height in pixels (matches camera output height)
    img_dsc.data_size = 480 * 320 * 2; // Total size of image data in bytes (480x320 resolution, 2 bytes per pixel e.g., RGB565)
    img_dsc.header.cf = LV_IMG_CF_TRUE_COLOR; // LVGL color format: true color (no palette, direct pixel values)
    img_dsc.data = NULL;               // Initialize image data pointer to NULL (will be set to camera buffer later)

    // Infinite loop to continuously capture and display camera frames


    while (1)
    {
        // Get a frame buffer from the camera (blocks until a frame is available)
        pic = esp_camera_fb_get();

        // Check if the frame buffer was successfully obtained
        if (NULL != pic)
        {
            // Assign the camera's frame data to the LVGL image descriptor
            img_dsc.data = pic->buf;

            // Lock LVGL to ensure thread-safe access (prevents UI glitches during rendering)
            if (lvgl_port_lock(0))
            {
                // Update the LVGL image object (cam_ing) with the new frame data
                lv_img_set_src(cam_ing, &img_dsc);
                // Unlock LVGL after updating the UI
                lvgl_port_unlock();
            }
        }
     
        // Return the frame buffer to the camera driver (allows reuse for next capture)
        esp_camera_fb_return(pic);

        // Short delay to reduce CPU usage and allow other tasks to run
...
void app_main(void)
{
    ...
    esp_camera_port_init(I2C_PORT_NUM);  // Initialize camera

    lv_port_init();                    // Initialize LVGL port (bind LCD/touch to LVGL framework)
    

    // Attempt to lock the LVGL port to ensure thread-safe access to LVGL UI components
    // (0 indicates no timeout; returns true if lock is acquired successfully)
    if (lvgl_port_lock(0))
    {
        // Create an LVGL image object (cam_ing) to display camera feed, parented to the current screen
        cam_ing = lv_img_create(lv_scr_act());
        
        // Set the size of the image object to 100% of the screen width and height (fullscreen)
        lv_obj_set_size(cam_ing, lv_pct(100), lv_pct(100));
        
        // Get a pointer to the camera sensor object (to check if camera is properly initialized)
        sensor_t *s = esp_camera_sensor_get();
        
        // If the camera sensor is successfully retrieved (not NULL), start the camera task
        if (s != NULL)
        {
            // Create a FreeRTOS task for camera frame capture and display
            // - Task function: camera_task
            // - Task name: "camera_task"
            // - Stack size: 10KB (1024 * 10 bytes)
            // - Parameters: NULL (no data passed to the task)
            // - Priority: 1
            // - Task handle: NULL (not tracking the handle)
            // - Core ID: 1 (pinned to CPU core 1)
            xTaskCreatePinnedToCore(camera_task, "camera_task", 1024 * 10, NULL, 1, NULL, 1);
        }
        else
        {     
            ...
            // Set the label display text, with the prompt content highlighted in red via embedded color code
            lv_label_set_text(label, "#ff0000  Connect Camera First. #");  
