...
esp_err_t Lcd_Init(void)
{
    ...
    esp_lcd_panel_dev_config_t panel_config = {
        ...
        .rgb_ele_order  = LCD_RGB_ELEMENT_ORDER_RGB,    /* BGR color element order */
...
static lv_obj_t* ui_create_center_label(void)
{
    lv_obj_t* label = NULL;
...
static void ui_show_label_text(lv_obj_t* label, const char* text)
{
...
void app_main(void)
{
    ...
    lv_obj_t * label =  ui_create_center_label();

    if (esp_sdcard_port_init() != ESP_OK) {
        ui_show_label_text(label, "No SD card detected. Please insert it and then power on again.");
        while (1) {
    ...
    lv_fs_if_init();
    speaker_Init();         // Initialize speaker hardware: configure I2S peripheral, amplifier, and audio output path

    /* Scan /sdcard/music directory for MP3 files */
    if (gui_hal_music_scan() != ESP_OK) {
        ui_show_label_text(label,
            "Please create a folder named \"music\" in the SD card root and copy your audio files into it.");
        while (1) {
            vTaskDelay(pdMS_TO_TICKS(1000));
        }
    }
        
    gui_hal_audio_init();   // Initialize audio player subsystem: register callbacks (volume/mute/play), create event queue, set default volume
    gui_music();
}