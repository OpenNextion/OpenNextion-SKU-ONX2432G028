...
/* Make img buttons look flat (remove theme shadow/background). */
...
static void make_imgbtn_flat(lv_obj_t * btn)
{
    /* LVGL theme might add bg/shadow/outline to (img)buttons.
     * For a 240x320 screen it can look like a "shadow" around the play button. */
    ...
    /* Some themes add a small transform on press; disable it for a crisp look. */
...
static lv_obj_t * create_cont(lv_obj_t * parent)
{
    ...
    lv_obj_set_scrollbar_mode(main_cont, LV_SCROLLBAR_MODE_OFF); /* Hide the right-side scrollbar */
...
static lv_obj_t * create_ctrl_box(lv_obj_t * parent)
{
    ...
    lv_obj_set_grid_cell(slider_obj,
                     LV_GRID_ALIGN_STRETCH, 1, 4,
                     LV_GRID_ALIGN_START,   1, 1);

    lv_obj_set_grid_cell(time_obj,
...
static void spectrum_anim_cb(void * a, int32_t v)
{
    ...
    /*
     * Album cover pulse (keep the ripple ring).
     * - Much smaller amplitude to prevent the cover from exceeding its container.
     * - Much shorter duration: pulse only on bass hits, otherwise decay back fast.
     */
    static int32_t zoom_ofs = 0;
    const int32_t ZOOM_MAX_OFS = 3;      /* +3/256 ≈ +1.17% */
    const int32_t BASS_TH      = 16;     /* higher threshold -> fewer pulses */
    const int32_t DECAY        = 6;      /* higher decay -> shorter pulse */

    int32_t bass_hit = (spectrum[spectrum_i][0] >= BASS_TH);
