            ...
            for (int batch_start = 0; batch_start < s_wifi_scan_num; batch_start += batch_size) {
                ...
                for (int i = batch_start; i < batch_end; i++) {
                    ...
                    if (gui_wifi.list_bnt[i]) {
                        ...
                        /* 240x320: slightly more compact list item height */
                        lv_obj_set_height(gui_wifi.list_bnt[i], 28);
        
                        lv_obj_t * bnt_label = lv_obj_get_child(gui_wifi.list_bnt[i], 0);
                        if(bnt_label != NULL)
                        {
                            lv_obj_set_height(bnt_label, 22);
                        }
...
static void wifi_switch_event_cb(lv_event_t *e)
{
    ...
    if (lv_obj_has_state(switch_obj, LV_STATE_CHECKED)) {
        ...
        lv_img_set_zoom(gui_wifi.img_wifi_state, 192);
        esp_wifi_scan_stop();
...
void wifi_gui_init(lv_obj_t *parent)
{
    ...
    if (lvgl_port_lock(portMAX_DELAY)) {
        /*
         * Target display: 240x320 (2.8")
         * Legacy UI was tuned for 240x320 (2.8").
         * - Image widgets are scaled down with lv_img_set_zoom(..., 192) (0.75x)
         * - Fixed sizes/offsets are reduced and derived from the parent size where possible
         */
    
        const int scr_w = lv_obj_get_width(parent);
        const int scr_h = lv_obj_get_height(parent);
        ...
        lv_obj_clear_flag(gui_wifi.cont_main,LV_OBJ_FLAG_SCROLLABLE);
        ...
        lv_img_set_src(gui_wifi.img_wifi_state, &wifi_closed_28);
        lv_img_set_zoom(gui_wifi.img_wifi_state, 192);
        lv_obj_align(gui_wifi.img_wifi_state, LV_ALIGN_TOP_RIGHT, -6, 6);
        

        /* Title container (WiFi label + switch) */
        gui_wifi.cont_title = lv_obj_create(gui_wifi.cont_main);
        lv_obj_set_size(gui_wifi.cont_title, scr_w - 20, 36);
        ...
        lv_obj_align(gui_wifi.cont_title, LV_ALIGN_TOP_MID, 0, 18);
        lv_obj_clear_flag(gui_wifi.cont_title,LV_OBJ_FLAG_SCROLLABLE);
        ...
        lv_obj_set_size (gui_wifi.switch_wifi_state, 48, 26);
        ...
        const int list_w = scr_w - 20;
        const int list_y = 80;
        const int list_h = (scr_h - list_y -30);
        lv_obj_set_size(gui_wifi.list_wifi_info, list_w, list_h);
        ...
        lv_obj_set_style_min_height(gui_wifi.list_wifi_info, 35, LV_PART_ITEMS);
        lv_obj_align(gui_wifi.list_wifi_info, LV_ALIGN_TOP_MID, 0, list_y);
        lv_obj_add_flag(gui_wifi.list_wifi_info, LV_OBJ_FLAG_HIDDEN);
        ...
        lv_obj_set_size(gui_wifi.spinner_wait, 60, 60);
        lv_obj_add_flag(gui_wifi.spinner_wait, LV_OBJ_FLAG_HIDDEN);

        
        /* WiFi connection dialog (password input) */
        gui_wifi.cont_wifi_link = lv_obj_create(lv_scr_act());
        lv_obj_set_size(gui_wifi.cont_wifi_link, lv_pct(100), lv_pct(100));
        ...
        lv_obj_set_size(gui_wifi.btn_return, 30, 25);
        lv_img_set_src(gui_wifi.btn_return, &left_handle);
        lv_img_set_zoom(gui_wifi.btn_return, 192);
        lv_obj_align(gui_wifi.btn_return, LV_ALIGN_TOP_LEFT, 6, 10);
        ...
        lv_obj_align(SSID_label2, LV_ALIGN_TOP_LEFT, 12, 52);

        gui_wifi.label_ssid = lv_label_create(gui_wifi.cont_wifi_link);
        lv_obj_set_width(gui_wifi.label_ssid, scr_w - 12 - 60);
        lv_label_set_long_mode(gui_wifi.label_ssid, LV_LABEL_LONG_DOT);
        lv_obj_align(gui_wifi.label_ssid, LV_ALIGN_TOP_LEFT, 60, 52);

        /* Password input container */
        lv_obj_t *ta_cont = lv_obj_create(gui_wifi.cont_wifi_link);
        lv_obj_set_size(ta_cont, scr_w - 20, 36);
        ...
        lv_obj_align(ta_cont, LV_ALIGN_TOP_MID, 0, 85);
        ...
        lv_obj_set_size(gui_wifi.keyboard, scr_w, 120);
        lv_obj_align(gui_wifi.keyboard, LV_ALIGN_BOTTOM_MID, 0, 0);
        lv_obj_set_style_text_font(gui_wifi.keyboard, &lv_font_montserrat_12, LV_PART_ITEMS);
        lv_obj_add_flag(gui_wifi.keyboard, LV_OBJ_FLAG_HIDDEN);
        ...
        lv_obj_set_style_height(gui_wifi.text_pwd, 36, LV_PART_MAIN);
        ...
        lv_obj_set_style_text_font(gui_wifi.text_pwd, &lv_font_montserrat_14, LV_PART_MAIN);
        ...
        lv_obj_align(gui_wifi.label_log, LV_ALIGN_CENTER, 0, 30);
        lv_obj_add_flag(gui_wifi.label_log, LV_OBJ_FLAG_HIDDEN);
