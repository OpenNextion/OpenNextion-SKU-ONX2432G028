...
void lcd_brightness_init(void)
{
    ...
    ledc_channel.duty = LCD_BL_LEDC_DUTY/2;          /* Initial PWM duty cycle */
...
/**
 * @brief LVGL-based touch screen test function
 * 
 * This function initializes a touch test UI: creates a prompt label, then enters an infinite loop
 * to read touch coordinates from the touch controller. When a touch is detected, it draws a small 
 * red square (4x4 pixels) at the touch position. It includes coordinate boundary checks to avoid
 * drawing outside the LCD screen range, and uses LVGL port locks to ensure thread-safe UI operations.
 */
void touch_test(void)
{
    uint16_t touchpad_x;                    // store touch X coordinate
    uint16_t touchpad_y;                    // store touch Y coordinate 
    uint16_t color_arr[16] = {0};          // Color buffer for drawing touch feedback (4x4 pixels = 16 pixels total)
    lv_obj_t *label = NULL;                // LVGL label object for displaying test prompt
    uint8_t touchpad_cnt = 0;              // Number of detected touch points (0 = no touch)

    // LV_COLOR_16_SWAP = 1  0x00f8 red
    for (int i = 0; i < 16; i++)
    {
        color_arr[i] = 0x00f8;
    }

    // Acquire LVGL port lock to ensure thread-safe UI operation
    if (lvgl_port_lock(0))
    {
        // Create a label on the current screen
        label = lv_label_create(lv_scr_act());
        lv_label_set_recolor(label, true);    // Enable text color recolor using #color code
        lv_label_set_text(label, "#808080 Touch Test#");  // Set label text (dark gray color)
        lv_obj_center(label);                 // Center the label on the screen

        lvgl_port_unlock();                   // Release LVGL port lock
    }

    vTaskDelay(pdMS_TO_TICKS(500));  // Delay 500ms to let the UI render stably

    if (lvgl_port_lock(0))
    {
        while (1)  // Infinite loop for continuous touch detection and feedback
        {
            /* Read raw data from the touch controller (internal data update) */
            esp_lcd_touch_read_data(touch_handle);

            /* Extract touch coordinates, press status and touch point count */
            // Parameters: touch handle, X coord buffer, Y coord buffer, Z coord (NULL = unused), touch count, max touch points
            bool touchpad_pressed = esp_lcd_touch_get_coordinates(touch_handle, &touchpad_x, &touchpad_y, NULL, &touchpad_cnt, 1);
            
            // Check if touch is detected and at least 1 valid touch point exists
            if (touchpad_pressed && touchpad_cnt > 0)
            {
                // Optional: Uncomment to flip X coordinate (adjust if touch is mirrored)
                // touchpad_x = EXAMPLE_LCD_H_RES - 1 - touchpad_x;

                /* Boundary check for X coordinate: prevent drawing outside screen */
                if (touchpad_x < 2)
                    touchpad_x = 2;  // Minimum X (avoid negative offset when drawing 4x4 square)
                else if (touchpad_x > LCD_H_RES - 2 - 1)
                    touchpad_x = LCD_H_RES - 2 - 1;  // Maximum X (LCD_H_RES = horizontal resolution)

                /* Boundary check for Y coordinate: prevent drawing outside screen */
                if (touchpad_y < 2)
                    touchpad_y = 2;  // Minimum Y (avoid negative offset when drawing 4x4 square)
                else if (touchpad_y > LCD_V_RES - 2 - 1)
                    touchpad_y = LCD_V_RES - 2 - 1;  // Maximum Y (LCD_V_RES = vertical resolution)

                /* Draw a 4x4 red square at the touch position */
                // Parameters: LCD panel handle, start X, start Y, end X, end Y, color buffer
                esp_lcd_panel_draw_bitmap(panel_handle, 
                                            touchpad_x - 2,  // Square top-left X (touch X - 2)
                                            touchpad_y - 2,  // Square top-left Y (touch Y - 2)
                                            touchpad_x + 2,  // Square bottom-right X (touch X + 2)
                                            touchpad_y + 2,  // Square bottom-right Y (touch Y + 2)
                                            color_arr);         // Red color data for the square
            }

            vTaskDelay(pdMS_TO_TICKS(5));  // 5ms delay to control touch detection refresh rate (~200Hz)
        }
    }
    lvgl_port_unlock();
}
...
void app_main(void)
{
    ...
    touch_test();                      // Start LVGL-based touch test (draws red squares on touch)
}