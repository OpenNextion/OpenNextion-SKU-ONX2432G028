...
static char s_img_path[MAX_FILENAME_LENGTH + 32];
...
static void ui_show_image(int idx)
{
    ...
    build_lvgl_image_path(s_img_index, s_img_path, sizeof(s_img_path));

    ESP_LOGI(TAG, " \r\n ****************** s_img_path =%s  *********** \r\n",s_img_path);
    
    ui_image_header_t header;
    if(ui_get_image_info(s_img_path, &header)) {
        UI_IMAGE_SET_SRC(img, s_img_path);
...
static void img_gesture_event_cb(lv_event_t *e)
{
    ...
    if(dir == LV_DIR_LEFT) {
         ESP_LOGI(TAG, "s_img_index = %d",s_img_index);
        s_img_index++;
...
void app_main(void)
{
    ...
    {  
        ...
        {    
            ...
            lv_obj_set_size(label, LCD_H_RES - 10, 100);
